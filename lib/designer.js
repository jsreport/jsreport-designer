...
class Designer {
  ...
  registerComponent (componentDefinition, componentModulePath) {
    ...
    if (componentModulePath != null) {
      fullComponentModulePath = require.resolve(componentModulePath)
      this.componentTypesModulePath[componentDefinition.name] = fullComponentModulePath
    }

    if (fullComponentModulePath) {
      // allowing importing component in jsreport helpers
      reporter.options.tasks.modules.push({
        alias: fullComponentModulePath,
        path: fullComponentModulePath
      })
    }
  }
