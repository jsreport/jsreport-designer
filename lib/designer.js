...
function renderDesignGroup (designGroup, options) {
  ...
  if (designGroup.topSpace != null) {
    designGroupContent += 'style="margin-top: ' + Handlebars.escapeExpression(designGroup.topSpace) + 'px" '
  }
...
function renderComponent (type, props, options) {
  var render = componentRegistry.getComponentFromType(type).render
  console.log(render)
  return new Handlebars.SafeString(render(props, options.data.__dataInput__))
}
