...
class Designer {
  ...
  registerComponent (componentDefinition) {
    ...
    let componentDefinitionToUse = { ...componentDefinition }
    const defaultStylePropName = 'style'

    reporter.logger.debug(`Registering component ${componentDefinitionToUse.name} for designer`)

    if (componentDefinitionToUse.propMeta && componentDefinitionToUse.propMeta[defaultStylePropName] === false) {
      ...
      delete componentDefinitionToUse.propMeta[defaultStylePropName]
    } else if (
      componentDefinitionToUse.propsMeta &&
      componentDefinitionToUse.propsMeta[defaultStylePropName] == null
    ) {
      // by default components have a style prop
      componentDefinitionToUse.propsMeta[defaultStylePropName] = {
        styleProp: true
      }
    }
  ...
  renderDesign (req, res) {
    ...
    if (
      isNaN(designPayload.grid.baseWidth) ||
      (typeof designPayload.grid.baseWidth !== 'string' && typeof designPayload.grid.baseWidth !== 'number')
    ) {
      throw new Error('design payload needs "grid.baseWidth" property to be a valid number, check the "design" object in "template" property of request body')
    }

    designPayload.grid.baseWidth = parseFloat(designPayload.grid.baseWidth)
    ...
    if (
      isNaN(designPayload.grid.rowHeight) ||
      (typeof designPayload.grid.rowHeight !== 'string' && typeof designPayload.grid.rowHeight !== 'number')
    ) {
      throw new Error('design payload needs "grid.rowHeight" property to be a valid number, check the "design" object in "template" property of request body')
    }

    designPayload.grid.rowHeight = parseInt(designPayload.grid.rowHeight, 10)
