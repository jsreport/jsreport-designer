  ...
  if (defToUse.fragments != null) {
    ...
    Object.keys(defToUse.fragments).forEach((fragName) => {
      ...
      if (defToUse.fragments[fragName].mode == null) {
        ...
        defToUse.fragments[fragName].mode = 'inline'
      } else if (
        defToUse.fragments[fragName].mode !== 'inline' &&
        defToUse.fragments[fragName].mode !== 'component'
      ) {
        throw new Error(`Invalid fragment mode, "${defToUse.fragments[fragName].mode}" is not a valid mode`)
      }
    })
  }
