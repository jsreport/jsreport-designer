...
Designer.prototype.parsePayload = function (req, res) {
  ...
  if (typeof designPayload.grid !== 'object' || (typeof designPayload.grid === 'object' && Array.isArray(designPayload.grid))) {
    throw new Error('design payload needs "grid" property to be an object, check the "designer" object in "template" property of request body')
  }
  ...
  if (
    isNaN(designPayload.grid.baseColWidth) ||
    (typeof designPayload.grid.baseColWidth !== 'string' && typeof designPayload.grid.baseColWidth !== 'number')
  ) {
  ...
  designPayload.grid.baseColWidth = parseFloat(designPayload.grid.baseColWidth)

  if (designPayload.components == null) {
...
Designer.prototype.createDesign = function (grid, components) {
  ...
  components.forEach(function (compGroup) {
    ...
    compGroup.group.forEach(function (comp) {
      ...
      compContent = ComponentType.template(
        assign(ComponentType.getDefaultProps(), comp.props)
      )
