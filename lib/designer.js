...
const generalProps = require('../shared/generalProps')
...
class Designer {
  constructor (reporter, definition, DEVELOPMENT) {
    ...
    reporter.options.tasks.modules.push({
      alias: 'generalProps.js',
      path: path.join(__dirname, '../shared/generalProps')
    })
      ...
      resolveResources
        .then(({ mainTemplate, mainHelpers, mainStyles }) => {
          ...
          if (req.template.helpers && typeof req.template.helpers === 'object') {
            ...
            req.template.helpers.generalProps = require(path.join(__dirname, '../shared/generalProps'))
...
function normalizePropsMetaDef (def, isRoot = true) {
  ...
  if (isRoot) {
    // adding general props to the component
    defToUse[childrenPropsName] = Object.assign(
      generalProps.getGeneralPropsMeta(),
      defToUse[childrenPropsName]
    )

    if (defToUse[childrenPropsName] == null) {
      defToUse[childrenPropsName] = {}
    }
  } else if (!defToUse[childrenPropsName]) {
    return defToUse
  }
  ...
  if (defToUse.fragments != null) {
    ...
    Object.keys(defToUse.fragments).forEach((fragName) => {
      const fragmentMode = defToUse.fragments[fragName].mode != null ? (
        defToUse.fragments[fragName].mode
      ) : 'inline'

      if (
        fragmentMode !== 'inline' &&
        fragmentMode !== 'component'
      ) {
        throw new Error(`Invalid fragment mode, "${fragmentMode}" is not a valid mode`)
      }

      if (fragmentMode === 'inline') {
        // only normalize props when fragment is inline
        defToUse.fragments[fragName] = normalizePropsMetaDef(defToUse.fragments[fragName], true)
      }

      // if mode was empty normalize it to default "inline".
      // safe to modify fragment here because we have
      // a copy here after "normalizePropsMetaDef"
      if (defToUse.fragments[fragName].mode == null) {
        // default to inline mode
        defToUse.fragments[fragName].mode = fragmentMode
      }
    })
  }
