...
(function (global, _inputs) {
  ...
  if (designInfo.computedFields) {
    computedFieldsFunctions = Object.keys(designInfo.computedFields).reduce((result, computedName) => {
      const computedFunction = evaluateScript.getSingleExport(designInfo.computedFields[computedName])
